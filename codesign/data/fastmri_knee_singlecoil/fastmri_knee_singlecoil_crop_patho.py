from codesign.data.fastmri_knee_singlecoil.fastmri_knee_singlecoil import get_valid_files_in_dir
...
class FSCPathoData(Dataset):
    ...
    def __init__(
        ...
        if pathologies is None or len(pathologies) == 0:
            ...
            for patho in pathologies:
                self.box_dict.update(knee_labels[FSCPathoData.common_patho_dict[patho]])
    ...
    def __getitem__(self, i):
        ...
        # 2x subsample along kx to get 1/2 FOV
        kspace = kspace[torch.arange(1, len(kspace), 2)]
        ...
        bbox_mask = self._box_to_mask(
            ...
            crop_shape=FSCPathoData.CROP_SHAPE,
    ...
    def process_annos(original_shape: List[int] = [320, 320]):
        ...
        for pathology_name in FSCPathoData.common_patho_list:
...
@dataclass
class FastMRIKneeSingleCoilCropPathoDataModule(BaseDataModule):
    ...
    def setup(self, stage: Optional[str] = None):
        ...
        self.train_set = FSCPathoData(
            train_files,
            self.shape,
            pathologies=self.pathologies 
        )
        self.val_set = FSCPathoData(
            val_and_test_files,
            self.shape, 
            custom_split='val',
            pathologies=self.pathologies 
        )
        self.test_set = FSCPathoData(
            val_and_test_files,
            self.shape, 
            custom_split='test',
            pathologies=self.pathologies 
...
if __name__ == '__main__':
    # FastMRIKneeSingleCoilPathoData.process_annos()
    a = FastMRIKneeSingleCoilCropPathoDataModule([320, 320], 24, pathologies=[0])
    a.setup()

    import matplotlib.pyplot as plt 
    for index in range(0, 100, 10):
        kspace, img, _, bbox_mask, _ =  a.test_set[index]

        plt.imshow(img[0], cmap='gray')
        plt.contour(bbox_mask[0])
        plt.savefig(f'tests/test_patho{index}.jpg')
        plt.close()