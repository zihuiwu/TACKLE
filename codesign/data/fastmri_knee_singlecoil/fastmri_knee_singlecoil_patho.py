...
class FastMRIKneeSingleCoilPathoData(Dataset):
    ...
    def __init__(
        ...
        if pathologies is None or len(pathologies) == 0:
            ...
            for patho in pathologies:
                self.box_dict.update(knee_labels[FastMRIKneeSingleCoilPathoData.common_patho_dict[patho]])
    ...
    def __getitem__(self, i):
        ...
        bbox_mask = self._box_to_mask(
            ...
            crop_shape=FastMRIKneeSingleCoilPathoData.CROP_SHAPE,
    ...
    def process_annos(original_shape: List[int] = [320, 320]):
        ...
        for pathology_name in FastMRIKneeSingleCoilPathoData.common_patho_list:
...
@dataclass
class FastMRIKneeSingleCoilPathoDataModule(BaseDataModule):
    ...
    def setup(self, stage: Optional[str] = None):
        ...
        self.train_set = FastMRIKneeSingleCoilPathoData(
            train_files,
            self.shape,
            pathologies=self.pathologies 
        )
        self.val_set = FastMRIKneeSingleCoilPathoData(
            val_and_test_files,
            self.shape, 
            custom_split='val',
            pathologies=self.pathologies 
        )
        self.test_set = FastMRIKneeSingleCoilPathoData(
            val_and_test_files,
            self.shape, 
            custom_split='test',
            pathologies=self.pathologies 
...
if __name__ == '__main__':
    ...
    a = FastMRIKneeSingleCoilPathoDataModule([192, 192], 24, pathologies=[1])
    a.setup()
