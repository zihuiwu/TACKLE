...
class FastMRIKneeSingleCoilCropData(Dataset):
    def __init__(
        self,
...
@dataclass
class FastMRIKneeSingleCoilCropDataModule(BaseDataModule):
    ...
    def setup(self, stage: Optional[str] = None):
        ...
        self.train_set = FastMRIKneeSingleCoilCropData(
            train_files,
            self.shape 
        )
        self.val_set = FastMRIKneeSingleCoilCropData(
            val_and_test_files,
            self.shape, 
            custom_split='val'
        )
        self.test_set = FastMRIKneeSingleCoilCropData(
            val_and_test_files,
            self.shape, 
            custom_split='test'
        )
...
class FastMRIKneeSingleCoilKFoldDataModule(BaseKFoldDataModule):
    ...
    def setup_fold_index(self, fold_index: int) -> None:
        ...
        self.train_fold = FastMRIKneeSingleCoilCropData(
            train_files,
            self.shape 
        )
        self.val_fold = FastMRIKneeSingleCoilCropData(
            val_files,
            self.shape 
        )
...
if __name__ == '__main__':
    ...
    for i in tqdm(range(len(a.train_set))):
        ...
        kspace = data[0]
        val = torch.view_as_real(kspace).norm(dim=-1).reshape(-1)
