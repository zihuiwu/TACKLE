...
class FastMRIKneeMultiCoilData(Dataset):
    def __init__(
        self,
...
@dataclass
class FastMRIKneeMultiCoilDataModule(BaseDataModule):
    ...
    def setup(self, stage: Optional[str] = None):
        ...
        self.train_set = FastMRIKneeMultiCoilData(
            train_files,
            self.shape 
        )
        self.val_set = FastMRIKneeMultiCoilData(
            val_and_test_files,
            self.shape, 
            custom_split='val'
        )
        self.test_set = FastMRIKneeMultiCoilData(
            val_and_test_files,
            self.shape, 
            custom_split='test'
        )
...
@dataclass
class FastMRIKneeMultiCoilKFoldDataModule(BaseKFoldDataModule):
    def __init__(self, shape, batch_size=24, kfold_seed=1234):
        super().__init__(shape, batch_size, kfold_seed)

    def setup(self, stage: Optional[str] = None) -> None:
        self.train_val_files = get_valid_files_in_dir('./datasets/knee/knee_multicoil_train', self.shape)
        self.test_files = get_valid_files_in_dir('./datasets/knee/knee_multicoil_val', self.shape)
    ...
    def setup_fold_index(self, fold_index: int) -> None:
        ...
        self.train_fold = FastMRIKneeMultiCoilData(
            train_files,
            self.shape 
        )
        self.val_fold = FastMRIKneeMultiCoilData(
            val_files,
            self.shape 
        )

if __name__ == '__main__':
    a = FastMRIKneeMultiCoilDataModule([192, 192], 24)
    a.setup()

    pathlib.Path("datasets/knee/knee_multicoil_train_slices_h5").mkdir(parents=True, exist_ok=True)
    pathlib.Path("datasets/knee/knee_multicoil_val_slices_h5").mkdir(parents=True, exist_ok=True)

    a.train_set.split_volume_into_slices("datasets/knee/knee_multicoil_train_slices_h5")
    a.val_set.split_volume_into_slices("datasets/knee/knee_multicoil_val_slices_h5")
    a.test_set.split_volume_into_slices("datasets/knee/knee_multicoil_val_slices_h5")